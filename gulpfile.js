...
//2.复制文件 - 不需要任何的包
gulp.task('copyfile', () => {
    return gulp.src('src/fonts/*') //载入src/fonts下面的所有文件
        .pipe(gulp.dest('dist/fonts/')); //输出。自动生成。
});
//3.压缩html文件。
...
gulp.task('default', function () {
    watch(['src/*.html', 'src/css/*.css', 'src/script/*.js', 'src/sass/*.scss'], gulp.parallel('uglifyhtml', 'uglifycss', 'uglifyjs', 'compilesass'));
});